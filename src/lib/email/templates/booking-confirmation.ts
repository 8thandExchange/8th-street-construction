...
const { parchment, ink, inkMuted, rust, pencil, border, borderLight, paper } = EMAIL_BRAND;
...
export function bookingConfirmationEmail(
  ...
  if (variant === "internal") {
    const inner = `
      <h2 style="font-family:${EMAIL_FONT.display};font-size:22px;margin:0 0 16px;font-weight:500;">${escapeHtml(b.first_name)} ${escapeHtml(b.last_name)} requested a consultation</h2>
      <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
        ${[
          ["Meeting Type", meeting],
          ["Preferred Date", b.preferred_date],
          ["Time Window", window],
          ["Email", `<a href="mailto:${b.email}" style="color:${rust};">${b.email}</a>`],
          ["Phone", `<a href="tel:${b.phone}" style="color:${rust};">${b.phone}</a>`],
          ...
          .map(
            ([k, v]) => `
          <tr>
            <td style="padding:10px 0;border-bottom:1px solid ${borderLight};font-size:12px;letter-spacing:0.1em;text-transform:uppercase;color:${pencil};width:140px;vertical-align:top;">${k}</td>
      ...
      ${
        b.notes
          ? `<div><div style="font-size:12px;letter-spacing:0.1em;text-transform:uppercase;color:${pencil};margin-bottom:8px;">Notes</div><div style="background:${paper};padding:16px;border-radius:4px;font-size:15px;line-height:1.6;white-space:pre-wrap;">${escapeHtml(b.notes)}</div></div>`
          : ""
      }
    `;
    return {
...
${b.notes ? `\nNotes: ${b.notes}` : ""}`,
  ...
  const inner = `
    <h1 style="font-family:${EMAIL_FONT.display};font-size:32px;font-weight:400;letter-spacing:-0.02em;line-height:1.15;margin:0 0 16px;">Consultation requested.</h1>
    <p style="font-size:16px;line-height:1.7;color:${inkMuted};margin:0 0 24px;">
    ...
    <div style="background:${paper};border:1px solid ${borderLight};padding:20px;border-radius:6px;margin:24px 0;">
      <div style="font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:${pencil};margin-bottom:12px;">Your request</div>
      <table style="width:100%;border-collapse:collapse;">
        <tr><td style="padding:6px 0;font-size:13px;color:${pencil};width:120px;">Meeting</td><td style="padding:6px 0;font-size:14px;">${meeting}</td></tr>
        <tr><td style="padding:6px 0;font-size:13px;color:${pencil};">Preferred Date</td><td style="padding:6px 0;font-size:14px;">${b.preferred_date}</td></tr>
        <tr><td style="padding:6px 0;font-size:13px;color:${pencil};">Time Window</td><td style="padding:6px 0;font-size:14px;">${window}</td></tr>
      </table>
    </div>
    <p style="font-size:16px;line-height:1.7;color:${inkMuted};">
