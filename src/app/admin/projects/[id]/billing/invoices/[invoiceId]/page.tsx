...
export default async function InvoiceDetailPage(props: {
  ...
  const [{ data: invoice }, { data: lines }, { data: attachments }, { data: budgetLines }] =
    await Promise.all([
      ...
      supabase
        ...
        .select(
          "id, description, quantity, unit_amount, amount, reference_number, city_budget_line_id, city_budget_line:city_budget_lines(city_number)"
        )
        .eq("invoice_id", invoiceId)
        .order("display_order"),
      supabase
        .from("invoice_attachments")
        .select("id, line_item_id, file_name")
        .eq("invoice_id", invoiceId)
        .order("display_order"),
      supabase
  ...
  const cityNumber = (li: (typeof lineList)[number]) => {
    const bl = Array.isArray(li.city_budget_line) ? li.city_budget_line[0] : li.city_budget_line;
    return bl?.city_number ?? null;
  };
            ...
            <div className="app-card overflow-hidden">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 border-t border-navy/[0.08] bg-navy/[0.02] p-6">
                <p className="text-sm app-muted max-w-md">
                  ...
                  first — and check every line has its backup invoice attached.
                </p>
                <div className="flex items-center gap-2">
                  <a
                    href={`/api/invoices/${invoiceId}/packet`}
                    target="_blank"
        ...
        <>
          <div className="app-card p-6 md:p-8">
            ...
            <ul className="divide-y divide-navy/[0.06]">
              {lineList.map((li) => (
                <li key={li.id} className="flex justify-between gap-4 py-2.5 text-sm text-navy/80">
                  <span>
                    {li.description}
                    {Number(li.quantity) !== 1 ? ` × ${Number(li.quantity)}` : ""}
                    {(li.reference_number || cityNumber(li) != null) && (
                      <span className="app-muted text-xs">
                        {li.reference_number ? ` · Inv. #${li.reference_number}` : ""}
                        {cityNumber(li) != null ? ` · City #${cityNumber(li)}` : ""}
                      </span>
                    )}
                  </span>
